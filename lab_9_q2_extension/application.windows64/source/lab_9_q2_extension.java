...
public class lab_9_q2_extension extends PApplet {
...
public void setup()
{
    // make taller so that text can fit without ever overlapping with actual drawing
  strokeWeight(2);
...
public float calcR(float degrees)
...
public void keyPressed()
{
  ...
  if (key == 'd') n2 = n2 < n2_MAX ? n2 + 0.1f : n2_MAX;
  if (key == 'a') n2 = n2 > 0 ? n2 - 0.1f : 0;
  if (key == 'w') n3 = n3 < n3_MAX ? n3 + 0.1f : n3_MAX;
  if (key == 's') n3 = n3 > 0 ? n3 - 0.1f : 0;
  if (key == 'l') a = a < a_MAX ? a + 0.1f : a_MAX;
  if (key == 'j') a = a > 0.1f ? a - 0.1f : 0.1f;  // a & b must always be a non-zero number; otherwise, the sketch won't show up (this is because of the superformula and the math behind it)
  if (key == 'i') b = b < b_MAX ? b + 0.1f : b_MAX;
  if (key == 'k') b = b > 0.1f ? b - 0.1f : 0.1f;
}
...
public void randomizeValues()
{
  ...
  public void settings() {  size(800, 845); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "lab_9_q2_extension" };
    if (passedArgs != null) {
